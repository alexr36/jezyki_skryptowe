...
class TimeSeries:
    ...
    def __init__(
            self, indic : str, code : str, 
            avg_time : str, timestamps : list[datetime],
            values : list[float], unit : str
        ) -> None:
        self.indic : str                 = indic
        self.code : str                  = code
        self.avg_time : str              = avg_time
        self.timestamps : list[datetime] = timestamps
        self.values : np.ndarray         = np.array(values, dtype=np.float64)
        self.unit : str                  = unit


    def __getitem__(self, key : int | slice | date) -> list | tuple | float:
    ...
    def _validate_values(self, vals : np.ndarray) -> np.ndarray:
